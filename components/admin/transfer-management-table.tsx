...
import type { TransferWithRelations } from "@/lib/types"

interface TransferManagementTableProps {
  transfers: TransferWithRelations[]
}

export function TransferManagementTable({ transfers }: TransferManagementTableProps) {
  ...
  const filteredTransfers = transfers.filter((transfer) => {
    const matchesSearch =
      ...
      transfer.beneficiary.name.toLowerCase().includes(searchTerm.toLowerCase())
  ...
  const exportToCSV = () => {
    const csvContent = [
      ["Date", "Sender", "Beneficiary", "Amount CAD", "Amount MGA", "Fee", "Status", "Type"].join(","),
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>Monitor and manage all money transfers</CardDescription>
          </div>
            ...
            <TableBody>
              {filteredTransfers.map((transfer) => (
                <TableRow key={transfer.id}>
                  ...
                  <TableCell>
                    <div>
                      <p className="font-medium">${transfer.amountCAD.toString()} CAD</p>
                      <p className="text-sm text-muted-foreground">{transfer.amountMGA.toLocaleString()} MGA</p>
                    </div>
                  </TableCell>
                  ...
                  <TableCell>
                    <Badge variant="outline">{transfer.type === "SUBSCRIPTION" ? "Recurring" : "One-time"}</Badge>
                  </TableCell>
