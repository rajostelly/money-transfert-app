...
async function getMadagascarDashboardData() {
  ...
  const pendingAmount = pendingTransfers.reduce((sum, transfer) => sum + Number(transfer.amountMGA), 0)
  ...
  return (
    <DashboardLayout>
      <div className="space-y-8">
        {/* Welcome Section */}
        <div>
          <h1 className="text-3xl font-bold text-foreground">Madagascar Team Dashboard</h1>
          <p className="text-muted-foreground mt-2">Confirm and manage money transfers to beneficiaries</p>
        </div>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          ...
          <StatsCard
            ...
            icon={CheckCircle}
          />
          <StatsCard title="Total Completed" value={data.stats.totalCompleted} description="All time" icon={Send} />
          <StatsCard
        ...
        <Card className="shadow-sm">
          <CardHeader>
            ...
            <CardDescription>Transfers waiting for confirmation</CardDescription>
          </CardHeader>
          <CardContent>
                    ...
                    <div className="flex-1">
                      <div className="flex items-center space-x-4">
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">From: {transfer.user.name}</p>
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center space-x-4">
                      <div className="text-right">
                        <p className="font-medium text-foreground">{transfer.amountMGA.toLocaleString()} MGA</p>
                        <p className="text-sm text-muted-foreground">${transfer.amountCAD.toString()} CAD</p>
                        <p className="text-sm text-muted-foreground">
